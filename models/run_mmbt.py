...
import pandas as pd
import numpy as np
from mmbt import MMBT_Model
import argparse
from config import Config, results_dir_mmbt
...
parser.add_argument('--txt_model_name', type=str, choices=["bert","bernice","bertweet"],help='model name')
...
parser.add_argument('--task', type=int, choices=[0,1,2,3,4,5,6,10], help='task to run')
...
# results directory
results_dir = results_dir_mmbt
if args.testing:
    results_dir += "testing/"
model_name = "mmbt"
logger.info("Model: {}, Task: {}, txt_model_name:{}, Epochs: {}, ITC loss: {}, TIM loss: {}, beta_itc: {}, beta_itm: {}, LC:{}, seed: {}".format(
    model_name, args.task, args.txt_model_name, args.epochs, args.use_clip_loss,
    args.use_tim_loss, args.beta_itc, args.beta_itm, args.use_loss_correction , args.seed))
# ------------------------------------------------------------------
...
def main():
    ...
    cfg = Config(args)
    # model
    mm_model = MMBT_Model(cfg, txt_model_name=args.txt_model_name, multilabel=cfg.multilabel)
    # load data    
